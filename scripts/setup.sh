...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --no-ingest)   NO_INGEST=true;   shift ;;
        --no-ctfd)     NO_CTFD=true;     shift ;;
        --noise-days)
...
BANNER
    ...
    echo -e "  ${CYAN}https://github.com/MrHacker-X/wazuhbots${NC}"
    echo ""
}
...
check_requirements() {
    ...
    if [[ "$(id -u)" -eq 0 ]] && command -v podman &>/dev/null; then
        ...
        case "${confirm}" in
            ...
            *) log_info "Aborted. Re-run without sudo."; exit 0 ;;
        esac
    fi
